...
const extractVideoThumb = async (path, time, size) => {
    // Lia@Changes 31-01-26 --- Use execFile instead of exec for ffmpeg
...
export const generateProfilePicture = async (mediaUpload, dimensions) => {
    ...
    if ('sharp' in lib && typeof lib.sharp?.default === 'function') {
        ...
        // Lia@Changes 31-01-26 --- Use ffmpeg via spawn as a fallback when sharp and jimp are not available
        const inputImgFilename = join(getTmpFilesDirectory(), generateMessageIDV2() + '.jpg');
        if (buffer) {
            await fs.writeFile(inputImgFilename, buffer);
        }
        const { stdout } = await execFileAsync('ffmpeg', [
            '-i', inputImgFilename,
            '-vf', `scale=${w}:${h}`,
        ...
        if (buffer) {
            await fs.unlink(inputImgFilename);
        }
        img = stdout;
    }
    return { img };
