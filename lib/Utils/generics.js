...
const baileysVersion = [2, 3000, 1032570163];
...
const getBufferedRandomBytes = (length) => {
...
export const generateMessageIDV2 = (userId) => {
    const ts = BigInt(unixTimestampSeconds());
    const random = getBufferedRandomBytes(16);
    const userData = userId ? (() => {
        const id = jidDecode(userId);
        return id?.user ? id.user + '@c.us' : '';
    })() : '';
    const hash = createHash('sha256')
        .update(Buffer.from(ts.toString(16).padStart(16, '0')))
        .update(userData)
        .update(random)
        .digest('hex')
        .toUpperCase();
    const pos = 4 + (random[0] & 15);
    const baseId = '3EB0' + hash.slice(0, 18);
    return baseId.slice(0, pos) + 'STARFALL' + baseId.slice(pos + 8);
};
